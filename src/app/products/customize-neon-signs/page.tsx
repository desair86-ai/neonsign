...
const COLORS = [
  ...
  { name: 'Green', hex: '#68ff7a', glow: 'rgba(104,255,122,0.5)' },
  { name: 'Blue', hex: '#245cff', glow: 'rgba(36,92,255,0.5)' },
  { name: 'Purple', hex: '#9b18ff', glow: 'rgba(155,24,255,0.5)' },
...
export default function CustomizeNeonSign() {
  const [text, setText] = useState('Neon Sign');
  const [selectedFont, setSelectedFont] = useState(FONTS[0]);
  const [selectedColor, setSelectedColor] = useState(COLORS[1]); // Pink default
    ...
    <main className="min-h-screen bg-black text-white font-sans selection:bg-pink-500/30 selection:text-pink-200">
      ...
      <div className="border-b border-white/10 bg-[#020202]">
        <div className="max-w-[1680px] mx-auto px-4 lg:px-8 py-5 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-sm font-bold uppercase tracking-[0.24em] text-[#cfff00]">Live custom neon builder</p>
          ...
          <div className="w-full">
            <div className="sticky top-28 overflow-hidden rounded-lg border border-white/10 bg-black min-h-[420px] lg:min-h-[720px] relative flex items-center justify-center p-6 md:p-10">
              ...
              <div className="absolute top-4 left-4 z-20 bg-white/10 backdrop-blur-md border border-white/20 text-white text-xs font-bold px-3 py-1.5 rounded-full flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-[#cfff00] animate-pulse"></span>
          ...
          <div className="w-full flex flex-col gap-7">
            ...
            <div className="space-y-3">
              <label className="text-xl font-black flex items-center justify-between">
                ...
                <span className="text-xs font-bold uppercase tracking-widest text-[#cfff00]">{text.length} chars</span>
              </label>
              <textarea 
                value={text}
                ...
                rows={2}
                className="w-full bg-black border border-white/80 rounded-md p-5 text-white placeholder:text-zinc-600 focus:outline-none focus:border-[#ff2aac] focus:ring-2 focus:ring-[#ff2aac]/60 transition-colors text-xl resize-none shadow-[0_0_24px_rgba(255,255,255,0.06)]"
              />
            </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {FONTS.map((font) => (
                  <button
                    ...
                    className={`min-h-16 py-3 px-2 rounded-md border text-center transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-[#cfff00] ${
                      selectedFont.name === font.name 
                        ? 'border-[#9b18ff] bg-[#9b18ff] text-white shadow-[0_0_28px_rgba(155,24,255,0.45)]' 
                        : 'border-white/70 bg-black text-zinc-200 hover:border-[#ff2aac] hover:text-white hover:shadow-[0_0_22px_rgba(255,42,172,0.18)]'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`rounded-md border flex flex-col focus:outline-none transition-colors overflow-hidden ${
                      selectedSize.id === size.id
                        ? 'border-[#8B26FF]'
                        : 'border-white hover:border-[#8B26FF]'
                    }`}
                  >
                    <div className={`w-full text-center py-2 font-black text-sm md:text-base ${
                      selectedSize.id === size.id ? 'bg-[#8B26FF] text-white' : 'bg-white text-black'
                    }`}>
                      {size.name}
                    </div>
                    <div className={`w-full text-center py-2.5 text-xs md:text-sm font-bold border-b ${
                      selectedSize.id === size.id ? 'bg-black text-white border-[#8B26FF]' : 'bg-black text-white border-white'
                    }`}>
                      Width: {size.width}
                    </div>
                    <div className={`w-full text-center py-2.5 text-xs md:text-sm font-bold ${
                      selectedSize.id === size.id ? 'bg-black text-white border-[#8B26FF]' : 'bg-black text-white'
                    }`}>
                      Height: {size.height}
                    </div>
                  </button>
                ...
                <label className="flex items-center justify-between gap-4 p-4 rounded-md border border-white/55 bg-black cursor-pointer hover:border-white transition-colors">
                  <div className="flex items-center gap-3">
                    <input 
                      ...
                      className="w-5 h-5 accent-[#ff2aac] rounded focus:ring-[#ff2aac]"
                    />
                    <div>
            ...
            <div className="sticky bottom-0 bg-black/95 backdrop-blur-md p-5 md:p-6 rounded-lg border border-white/30 mt-2 shadow-[0_0_48px_rgba(207,255,0,0.12)] z-40">
              <div className="flex items-end justify-between mb-6">
                <div>
                  <div className="text-zinc-400 text-sm font-medium mb-1">Total Price (Inc. GST)</div>
                  <div className="text-4xl md:text-5xl font-black text-[#cfff00] drop-shadow-[0_0_14px_rgba(207,255,0,0.65)]">
              ...
              <button className="w-full bg-[#ff2aac] hover:bg-[#ff48ba] text-white font-black py-4 rounded-md text-lg transition-colors flex items-center justify-center gap-2 shadow-[0_0_28px_rgba(255,42,172,0.45)]">
                Add to Cart
              </button>
            </div>
      ...
      <div className="max-w-[1200px] mx-auto px-4 py-16 space-y-24">
        ...
        <section id="about" className="scroll-mt-32">
          <h2 className="text-2xl font-bold text-[#cfff00] mb-4">About Your Neon Sign:</h2>
          <p className="text-zinc-200 text-lg mb-8 max-w-4xl">
            Neon Sign's neon signs are handcrafted with advanced 2nd gen LED on high-quality 6MM transparent acrylic. Energy-efficient, durable, and easy to install—perfect for any space!
          </p>
        ...
        <section id="box" className="scroll-mt-32 border-t border-white/10 pt-16">
          <h2 className="text-2xl font-bold text-[#cfff00] mb-4">The Box Contains:</h2>
        ...
        <section id="install" className="scroll-mt-32 border-t border-white/10 pt-16">
          <h2 className="text-2xl font-bold text-[#cfff00] mb-10">Here's how you can install our neon signs on your wall:</h2>
        ...
        <section className="scroll-mt-32 border-t border-white/10 pt-16 pb-16">
          <div className="bg-white rounded-3xl overflow-hidden flex flex-col md:flex-row shadow-2xl">
            ...
            <div className="w-full md:w-1/2 p-10 lg:p-16 flex flex-col justify-center">
              ...
              <button className="bg-[#66d366] hover:bg-[#52c152] text-black font-bold py-4 px-8 rounded-full text-lg w-max flex items-center gap-3 transition-colors shadow-xl">
